...
Reproduction must preserve the system under investigation. Prefer read-only discriminating probes over manufacturing a failure by removing packages, corrupting state, weakening controls, or damaging a working environment. Label remediation commands as unexecuted until they actually run; never let a hypothetical fix masquerade as execution evidence.
