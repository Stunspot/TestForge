...
TestForge v1.1.5 is a local, skills-only plugin. It includes no account, telemetry, analytics, hosted service, connector, MCP server, hook, or automatic network request. Collaborative Dynamics does not receive repositories, diffs, prompts, logs, test data, or generated outputs through the plugin.
...
This statement describes the public v1.1.5 package as built. Any future connector, hosted service, telemetry, or tool-backed edition requires a new privacy review and an updated statement before release.
